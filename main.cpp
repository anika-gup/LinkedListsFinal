...
void ADD(Node* node, Node* &head, Node* current, Node* previous) {
  ...
  if (head==NULL) {
    head = node;
    // cout << "head is: " << head->getStudent()->getID() << endl;
    return;
  }
  // if student is first
  ...
  // if student is last
    ...
    if (node->getStudent()->getID() < current->getStudent()->getID()) {
      ...
      Node* filler = node;
      previous->setNext(filler);
      filler->setNext(current);
    }
    else {

      Node* filler = node;
      current->setNext(filler);
      filler->setNext(NULL);
    }
  }
  else {
    ...
    // keep iterating forward unless it finds where it goes
    if (node->getStudent()->getID() < current->getStudent()->getID() ) {
      //Node* filler = node;
      previous->setNext(node);
      node->setNext(current);
      //  cout << "added: " << node->getStudent()->getID() ;
    }
    else {
      //  cout << "head is: " << head->getStudent()->getID() << endl;
      ADD(node, head, current->getNext(), current) ;
    }
  }

    

    // last case, in middle of list
...
void PRINT(Node* head, Node* current) {
  ...
  if (current==head) {
    ...
    cout << "head id: " << current->getStudent()->getID() << endl;
  }
  cout << "current id is: " << current->getStudent()->getID() << endl;
